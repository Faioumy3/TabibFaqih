import { Teacher, Student, AttendanceRecord, StudentLog, ExportData, Message, Conversation } from '../types';
...
export const api = {
  ...
  exportData: async (): Promise<ExportData> => {
    ...
    const messages = mSnap.docs.map(d => d.data() as Message);
    ...
    return { teachers, students, attendance, studentLogs, messages };
  },

  // Messages API
  sendMessage: async (message: Omit<Message, 'id'>): Promise<string> => {
  ...
  getSentMessages: async (userId: string): Promise<Message[]> => {
    const q = query(
      collection(db, 'messages'),
      where('from', '==', userId)
    );
  ...
  getConversation: async (userId1: string, userId2: string): Promise<Message[]> => {
    ...
    const messages = [
      ...snap1.docs.map(d => ({ ...d.data() as Message, id: d.id })),
      ...snap2.docs.map(d => ({ ...d.data() as Message, id: d.id }))
    ];
